...
- **Ping-pong privremeni buffer (`temp[]`)**  
  Umjesto da se pri svakom merge-u alocira novi lokalni buffer, koristi se **jedan privremeni buffer (`temp`)** koji se u bottom-up merge fazi **izmjenjuje sa originalnim nizom (`arr`)** kroz swap pokazivača (`src` ↔ `dest`).  
  **Efekat:** smanjuje broj memorijskih alokacija, poboljšava cache lokalnost i omogućava brži merge.

- **Insertion sort za male segmente (`≤16` elemenata)**  
  Za male segmente (manje od `INSERTION_SORT_THRESHOLD`, empirijski odabrano 16) koristi se `insertion_sort`, koji je brži od merge sort-a zbog minimalnog overhead-a iteracija i jednostavnih poređenja.  
  **Efekat:** brže sortiranje malih blokova i smanjenje ukupnog broja operacija.

- **Preskakanje merge-a kada su segmenti već sortirani**  
  Provjerava se da li je **zadnji element lijevog segmenta manji ili jednak prvom elementu desnog** (`src[m] <= src[m+1]`). Ako jeste, merge se preskače, a cijeli segment se kopira u dest buffer.  
  **Efekat:** smanjuje nepotrebne operacije merge-a, posebno za skoro sortirane ili segmentno sortirane nizove.

- **Efikasno kopiranje blokova memorije (`memcpy`)**  
  Umjesto element-po-element kopiranja, segmenti koji ne zahtijevaju merge se kopiraju **blokovima memorije**.  
  **Efekat:** smanjeno opterećenje CPU-a i brže kopiranje podataka.

- **Ukupni efekat optimizacija:**
...
### Benchmark (vrijeme izvršavanja u ms)
...
**Zaključak:**  
Optimizovana sekvencijalna implementacija merge sorta pruža **značajno ubrzanje** u odnosu na naivnu verziju, posebno za velike i skoro sortirane nizove. Ovo je idealna osnova za **paralelizaciju na CPU/GPU**.
...
NESTO TREBA IZVRSAVATI U P CORE U TASK MANAGER.
